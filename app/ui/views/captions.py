...
class CaptionLibraryView(QWidget):
	def __init__(self, config: AppConfig, parent: QWidget | None = None) -> None:
		...
		self._text.setPlaceholderText("Caption text (use {LINK}, {GROUP} etc.)")
		self._category = QLineEdit()
		self._category.setPlaceholderText("Category (optional)")
		self._tags = QLineEdit()
		self._tags.setPlaceholderText("Tags JSON (optional)")
		self._uniq = QLineEdit()
		self._uniq.setPlaceholderText("Uniqueness mode: none/spin/append-random (optional)")
		self._add = QPushButton("Add Caption")
		self._del = QPushButton("Delete Selected")

		head = QHBoxLayout()
		head.addWidget(self._text)
		head.addWidget(self._category)
		head.addWidget(self._tags)
		head.addWidget(self._uniq)
